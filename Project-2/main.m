...
% -------CALCULATE INVERSE AND DETERMINANT OF COVARIANCE MATRICES FOR
% FUTURE USE: ---------

sigma_inv = zeros(d,d,c);
sigma_det = zeros(1,c);

for i = 1:c
    
    sigma_inv(:,:,i) = inv(covariance_matrices(:,:,i));
    sigma_det(1,i) = det(covariance_matrices(:,:,i));
...
for i=1:c
    ...
    for j = 1:D_c75(i)
        ...
        for k = 1:c
            ...
            pred75(k,j) = D_c75(k) * MultivariateGaussian(data75{1,i}(:,j),mean(:,k),sigma_det(1,k),sigma_inv(:,:,k),d);
            
        end
        
    end
    
    predictions75(1,i) = { rem(find(pred75 == max(pred75)),c) }; % in this cell, we store the predictions for each training samples
    
    for j = 1:D_c25(i)
       
        for k = 1:c
           
            pred25(k,j) = D_c75(k) * MultivariateGaussian(data25{1,i}(:,j),mean(:,k),sigma_det(1,k),sigma_inv(:,:,k),d);
            
        end
        
    end
    
    predictions25(1,i) = { rem(find(pred25 == max(pred25)),c) }; % in this cell, we store the predictions for each test samples
