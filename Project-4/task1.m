...
% ------- MAKE THE CLASS PREDICTIONS on test set By Parzen Window: -------

n = [10, 50]; % use 2 different number of samples
h1 = [.5,1];    % use 2 different number of window size

predictions25 = cell(size(n,2),size(h1,2),c); % store the all the estimations of test set on this cell

for n_count = 1:size(n,2)
    
    for h_count = 1:size(h1,2)
        ...
        for i=1:c
            ...
            for j = 1:D_c25(i)
                ...
                for k = 1:c
                    ...
                    pred25(k,j) = decision_functionParzen(data25{1,i}(:,j), data75{1,k}(:,1:n(n_count)) ,D_c75(k));
                    
                end
                
            end
            predictions25(n_count,h_count,i) = { rem(find(pred25 == max(pred25)),c) }; % in this cell, we store the predictions for each test samples
...
for i = 1:c
    ...
    T_pos_test(1,i) = sum(rem(i,c) == predictions25{n_count,h_count,i});
    
    F_neg_test(1,i) = sum(rem(i,c) ~= predictions25{n_count,h_count,i});
    
    for j = 1:c
        Predicted_pos_test(1,i) = Predicted_pos_test(1,i) + sum(rem(i,c) == predictions25{n_count,h_count,j});
    end
